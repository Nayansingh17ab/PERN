...
function ClothingPage() {
  ...
  const [currentSubcategory, setCurrentSubcategory] = useState("shirts");
  ...
  const subcategoryTabs = [
    ...
    { id: "innerwears", name: "Inner Wears", icon: "🩲" }
  ];

  const fetchProducts = async (subcategory) => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:3000/api/clothing/${subcategory}`);
  ...
  const deleteProduct = async (id, subcategory) => {
    try {
      const response = await fetch(`http://localhost:3000/api/clothing/${subcategory}/${id}`, {
        method: "DELETE",
        credentials: "include"
      });
  ...
  const handleAddProduct = () => {
    if (!isAuthenticated || !isAdmin()) {
      ...
      navigate("/login", { state: { from: `/clothing` } });
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h1 className="text-4xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
            Clothing - {subcategoryTabs.find(t => t.id === currentSubcategory)?.name}
          </h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {products.map((product) => (
            <ProductCard key={product.id} product={product} subcategory={currentSubcategory} department="clothing" onDelete={deleteProduct} />
          ))}
        </div>
      )}
